...
let header=document.querySelector(".header"),navList=document.querySelector(".header-nav"),burger=document.querySelector(".header-burger");burger.addEventListener("click",()=>{header.classList.toggle("header--active"),document.body.classList.toggle("body--active"),navList.classList.toggle("header-nav--active"),burger.classList.toggle("header-burger--active")});
const label=document.querySelector(".label"),labelInput=document.querySelector(".label-input"),ico=document.querySelector(".label-svg"),text=document.querySelector(".label-text");let fileList;labelInput&&labelInput.addEventListener("change",(function(e){fileList=[];for(let e=0;e<labelInput.files.length;e++)fileList.push(labelInput.files[e]);fileList.forEach(e=>{uploadFile(e)})}));const uploadFile=e=>{fileList.length>0&&(ico.style.display="none",label.classList.add("label--active")),e&&fileList.length>1?(fileList.length<=4&&(text.textContent=`Выбрано ${fileList.length} файла`),fileList.length>4&&(text.textContent=`Выбрано ${fileList.length} файлов`)):text.textContent=e.name};
